    ...
    try {
        ...
        foreach ($parts as $p) {
            ...
            $pdo->prepare("
                ...
                (ng_id,part_code,lot_no,used_qty,ng_qty,ng_type,ref_part,shift,line_id)
                VALUES (?,?,?,?,?,?,?,?,?)
            ")->execute([
                $ngId,
                $part,
...
if ($action == 'in_meca') {
    ...
    try {
        ...
        if (!$p) throw new Exception("Product tidak ditemukan");
        ...
        if (!$ngRow) throw new Exception("Product belum EXIT MECA");
        ...
        if ($ppRow) {
            ...
            $shiftStart = $pdo->prepare("SELECT start FROM tbl_shift WHERE shift=?");
            $shiftStart->execute([$shift]);
            $start = intval($shiftStart->fetchColumn());
            ...
            $slots = $pdo->prepare("
                ...
                END
            ");
            $slots->execute([$ppRow['pp_id'], $start]);
